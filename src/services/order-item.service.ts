...
export class OrderItemService {
  ...
  async create(orderId: number, productIds: number[]): Promise<void> {
    const orderItems: CreateOrderItemDto[] = await Promise.all(productIds.map(async (productId) => {
      const product: ProductInstance = await this.productService.findByPk(productId);
